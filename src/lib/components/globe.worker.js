...
loadWASM().then((_SGP4) => {
    ...
    const celestrackUrl = `https://cors-noproblem.herokuapp.com/${datasource}`; // https://api.allorigins.win/raw?url=${datasource}

    fetch(celestrackUrl) // https://www.celestrak.com/norad/elements/active.txt : ./src/assets/starlink.txt
        .then((r) => r.text())
        .then((rawData) => {
            ...
            let tleTxt = '';
            tleData.filter(([name, ...tle]) => {
                    return /^(STARLINK|ONEWEB)/.test(name.trim().replace(/^0 /, ""))
                }).forEach(([name, ...tle]) => {
                    tleTxt += `\n${tle[0]}\n${tle[1]}`;
                })
            // Creates the internal satrecs
            //console.log('tleTxt', (tleTxt.trim()));
            SGP4.init(tleTxt.trim());
        }).then(() => {
...
function tick(time, from, satLog, satTable) {
    ...
    let obsResVector;
    if(from) {
        obsResVector = SGP4.observe_at(from, UTCFullYear, UTCMonth + 1, UTCDate, UTCHours, UTCMinutes, UTCSeconds);
    } else {
        obsResVector = SGP4.at(UTCFullYear, UTCMonth + 1, UTCDate, UTCHours, UTCMinutes, UTCSeconds);
    }

    //const ecis = SGP4.predict2(satrecs, UTCFullYear, UTCMonth + 1, UTCDate, UTCHours, UTCMinutes, UTCSeconds);
    //console.log('satrecs.size()', satrecs.size())
    for(let i = 0; i < obsResVector.size(); i++) {
        const { eci, geodetic, lookAngles, eclipse_status } = obsResVector.get(i);
        //console.log('eci', eci)
        let d = satData[i];
        //const eci = SGP4.predict(d.satrec, UTCFullYear, UTCMonth + 1, UTCDate, UTCHours, UTCMinutes, UTCSeconds);//satellite.sgp4(d.satrec, m);
        
        if(geodetic.longitude) {
            //const gdPos = SGP4.eciToGeodetic(eci.pos, gmst);
            d.lat = geodetic.latitude
            d.lng = geodetic.longitude//satellite.radiansToDegrees(gdPos.longitude);
            d.alt = geodetic.height // / EARTH_RADIUS_KM;
            ...
            if(from) {
                ...
                if(lookAngles.elevation * (180/Math.PI) >= from.minElevation) {
                    ...
                    transit.sunlit.push(!eclipse_status.eclipsed);
                    transit.eclipseDepth.push(eclipse_status.depth / deg2rad);
                    if(d.colorIndex != 11) {
